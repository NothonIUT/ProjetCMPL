...
public class PtGen {
 ...
 // MERCI de renseigner ici un nom pour le trinome, constitue EXCLUSIVEMENT DE LETTRES
	...
	public static void initialisations() {
		...
		// initiatlisation du nombre de variables globales dÈclarÈes
		cptVarGlobales = 0;
	} // initialisations
	...
	public static void pt(int numGen) {
		...
		case 1 : // consts
			if(presentIdent(1) == 0){
				...
				UtilLex.messErr("Constante dÈj‡ dÈclarÈe." );
			}
			break;
		...
		case 2 :
			if(presentIdent(1) == 0) {
				...
				UtilLex.messErr("Variable dÈj‡ dÈclarÈe");
			}
			break;
		...
		// expression
		// Les case 11 et 15 sont utilisÈs pour vÈrifier que l'expression est bien de type boolÈenne ou entiËre
		...
		// Production des opÈrations entre expressions
		case 12 :
			po.produire(OU);
			break;
		...
		case 27 : // ident
			// On vÈrifie que l'ident lu existe dans la table
			int id_ident = presentIdent(1);
			if (id_ident == 0) {
				UtilLex.messErr("L'ident utilisÈ n'existe pas");
			}
			...
			switch(tabSymb[id_ident].categorie) {
				...
				// Il reste les variables locales ‡ gÈrer
			}
		...
		case 28 :
			// VÈrifier que la derniËre expression Ètait boolÈenne
			verifBool();
			// Produire un bsifaux
			po.produire(BSIFAUX);
		...
		// Fin Tant que
		case 32 :
			int sortie_ttq = pileRep.depiler(); // Valeur d'ipo pour sortir du ttq (empilÈe au case 28)
			int debut_ttq = pileRep.depiler(); // Valeur d'ipo pour reboucler (empilÈe au case 30)
